...
def _raise_supabase_error(action: str, error: str | None) -> None:
    if error:
        print(f"[Supabase] Warning {action}: {error}")


def _proposal_terms(proposal: dict) -> dict:
    return {
        "assessmentId": proposal.get("assessment_id"),
        "tenant": proposal.get("tenant"),
        "landlord": proposal.get("landlord"),
        "system": proposal.get("system"),
        "consumption": proposal.get("consumption"),
    }
...
def _extract_property(address_raw: str | AddressModel | dict) -> dict:
    ...
    if sb:
        result = sb.from_("properties").insert([
            {
                "id": new_prop["id"],
                "name": new_prop["name"],
        ...
        _raise_supabase_error("inserting property", result.error)

    return new_prop


def _proposal_from_supabase(proposal_id_or_token: str) -> ProposalResponse | None:
    sb = get_supabase_client()
    if not sb:
        return None
    ...
    res = (
        sb.from_("proposals")
        .select("*")
        .or_(f"id.eq.{proposal_id_or_token},invite_token.eq.{proposal_id_or_token}")
        .limit(1)
        .execute()
    )
    _raise_supabase_error("fetching proposal", res.error)
    if not res.data:
        return None

    row = res.data[0]
    terms = row.get("terms") or {}
    prop_res = sb.from_("properties").select("*").eq("id", row["property_id"]).limit(1).execute()
    _raise_supabase_error("fetching proposal property", prop_res.error)
    ...
    return ProposalResponse(
        ...
        assessment_id=terms.get("assessmentId") or terms.get("assessment_id"),
        title=row.get("title", ""),
...
def create_proposal(payload: CreateProposalRequest) -> ProposalResponse:
    ...
    sb = get_supabase_client()
    if sb:
        result = sb.from_("proposals").insert([
            {
                "id": proposal_id,
                "property_id": property_rec["id"],
                "proposal_type": "solar",
        ...
        _raise_supabase_error("inserting proposal", result.error)
...
def get_proposal(proposal_id_or_token: str) -> ProposalResponse:
    ...
    proposal = _proposal_from_supabase(proposal_id_or_token)
    if proposal:
        return proposal
...
def accept_proposal(proposal_id_or_token: str, payload: AcceptProposalRequest) -> ProposalResponse:
    ...
    if not found_prop:
        ...
        if not proposal:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Proposal not found")
        found_prop = proposal.model_dump()
        found_prop["status"] = "accepted"
        found_prop["landlord"] = {"name": payload.landlord_name, "email": payload.landlord_email}
    ...
    if sb:
            ...
            .update(
                {
                    "status": "accepted",
                    "terms": {
                        **_proposal_terms(found_prop),
                        "landlord": {
                            "name": payload.landlord_name,
                            "email": payload.landlord_email,
                        },
                    },
                }
        ...
        _raise_supabase_error("updating proposal acceptance", result.error)
