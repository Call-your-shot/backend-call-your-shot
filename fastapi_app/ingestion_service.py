...
# In-memory database store for ingested raw telemetry packets
# Structured to map 1-to-1 with PostgreSQL database tables (raw_iot_telemetry_packets)
...
async def ingest_raw_telemetry(date_str: str, weather: str) -> IngestTelemetryResponse:
    ...
    and store raw hardware telemetry packets into the database.
    """
...
def query_raw_telemetry(date_str: Optional[str] = None) -> RawTelemetryQueryResponse:
    """
    Query raw IoT telemetry packets stored in database.
    Filterable by date.
    """
    if date_str:
        records = [r for r in RAW_TELEMETRY_DB if r["date"] == date_str]
    else:
        records = list(RAW_TELEMETRY_DB)

    telemetry_packets = [TelemetryPacket(**r["raw_payload"]) for r in records]
    return RawTelemetryQueryResponse(
        date=date_str,
        count=len(telemetry_packets),
        data=telemetry_packets,
    )
