...
export interface EnergyReadingRow {
  ...
  solar_consumed_by_tenant_kwh: number | null;
...
export interface GreenCreditProgramRow {
  id: string;
  property_id: string;
  name: string;
  version: number;
  credits_per_kwh_microcredits: number;
  tenant_share_bps: number;
...
export interface GreenCreditAccountRow {
  id: string;
  user_id: string;
...
export interface GreenProjectRow {
  id: string;
  slug: string;
  title: string;
  description: string;
  ...
  target_microcredits: number;
  funded_microcredits: number;
  minimum_allocation_microcredits: number;
  status: "draft" | "open" | "funded" | "active" | "completed" | "cancelled";
  impact_unit: string;
...
export type Database = {
  public: {
    Tables: {
      ...
      bills: Table<BillRow>;
      green_credit_programs: Table<GreenCreditProgramRow>;
      green_credit_accounts: Table<GreenCreditAccountRow>;
      green_projects: Table<GreenProjectRow>;
      green_project_allocations: Table<{ id: string; project_id: string; account_id: string; user_id: string; requested_microcredits: number; allocated_microcredits: number; status: "confirmed" | "refunded"; idempotency_key: string; allocated_at: string; refunded_at: string | null }>;
      green_credit_ledger_entries: Table<{ id: string; account_id: string; entry_type: "earn" | "allocate" | "refund" | "adjustment"; amount_microcredits: number; property_id: string | null; program_id: string | null; source_energy_reading_id: string | null; project_id: string | null; allocation_id: string | null; source_solar_kwh: number | null; beneficiary_role: PropertyRole | null; description: string; metadata: Json; occurred_at: string; created_at: string }>;
    ...
    Views: {
      green_credit_wallets: Table<{ account_id: string; user_id: string; status: "active" | "suspended" | "closed"; available_microcredits: number; lifetime_earned_microcredits: number; lifetime_allocated_microcredits: number; created_at: string; updated_at: string }>;
      green_project_funding: Table<GreenProjectRow & { remaining_microcredits: number }>;
    };
    Functions: {
      ...
      accrue_green_credits: { Args: { target_property_id: string; period_start: string; period_end: string }; Returns: Json };
      allocate_green_credits: { Args: { target_project_id: string; requested_microcredits: number; request_idempotency_key: string }; Returns: Json };
    };
    Enums: {
